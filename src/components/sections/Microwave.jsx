...
function AnimCanvas({ modeRef }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      t += 0.025;
      const label = modeRef?.current || '微波加热 · 磁控管 2.45GHz';
      ...
      ctx.fillText('磁控管、高压变压器与门开关联锁', W/2, H - 12);
...
export default function Microwave() {
  const modeRef = useRef('微波加热 · 磁控管 2.45GHz');
  return (
    <section id="microwave" className="sec">
      <div className="sh">
        <span className="sh-icon">📻</span>
        <div>
          <div className="sh-tag">MICROWAVE · 2kV</div>
          <h2 className="sh-title" style={{ color: ACC, textShadow: `0 0 35px ${ACC}44` }}>微波炉</h2>
          <p className="sh-sub">磁控管、高压变压器与门开关联锁</p>
        </div>
      </div>
      ...
      <div className="grid2">
        ...
        <div className="info-stack reveal">
          <div className="icard" style={{ borderLeftColor: ACC }}>
            ...
            <div style={{ fontSize: 13.5, color: '#aabfc8', lineHeight: 1.65 }}>磁控管、高压变压器与门开关联锁</div>
          </div>
          <div className="icard" style={{ borderLeftColor: ACC }}>
