...
function AnimCanvas({ modeRef }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      t += 0.025;
      const label = modeRef?.current || 'IH 涡流 · 锅体自身发热';
      ...
      ctx.fillText('IH 涡流加热、锅具检测与过温保护', W/2, H - 12);
...
export default function Induction() {
  const modeRef = useRef('IH 涡流 · 锅体自身发热');
  return (
    <section id="induction" className="sec">
      <div className="sh">
        <span className="sh-icon">🍳</span>
        <div>
          <div className="sh-tag">INDUCTION COOKER</div>
          <h2 className="sh-title" style={{ color: ACC, textShadow: `0 0 35px ${ACC}44` }}>电磁炉</h2>
          <p className="sh-sub">IH 涡流加热、锅具检测与过温保护</p>
        </div>
      </div>
      ...
      <div className="grid2">
        ...
        <div className="info-stack reveal">
          <div className="icard" style={{ borderLeftColor: ACC }}>
            ...
            <div style={{ fontSize: 13.5, color: '#aabfc8', lineHeight: 1.65 }}>IH 涡流加热、锅具检测与过温保护</div>
          </div>
          <div className="icard" style={{ borderLeftColor: ACC }}>
