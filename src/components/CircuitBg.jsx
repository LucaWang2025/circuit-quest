...
export default function CircuitBg({ active = true, theme = 'dark' }) {
  ...
  useEffect(() => {
    ...
    const lineRgb = theme === 'light' ? '0,100,180' : '0,229,255';
    ...
    function frame() {
      ...
      segs.forEach((s, i) => {
        ...
        cx.strokeStyle = `rgba(${lineRgb},${breathe})`;
        cx.lineWidth = 1;
        cx.beginPath(); cx.moveTo(s.x1, s.y1); cx.lineTo(s.x2, s.y2); cx.stroke();
      });

      cx.fillStyle = `rgba(${lineRgb},0.1)`;
      const drawn = new Set();
      segs.forEach(s => {
      ...
      dots.forEach(d => {
        ...
        g.addColorStop(0, `rgba(${lineRgb},0.4)`);
        g.addColorStop(0.5, `rgba(${lineRgb},0.12)`);
      ...
      for (let i = pulses.length - 1; i >= 0; i--) {
        ...
        cx.strokeStyle = `rgba(${lineRgb},${alpha})`;
        cx.lineWidth = 1;
